...
#include <QtCore>
#include <QtWidgets>
#include "symbolpolygongraphicsitem.h"
#include "symbolpin.h"
...
SymbolPolygonGraphicsItem::SymbolPolygonGraphicsItem(const SymbolPin& pin,
...
SymbolPolygonGraphicsItem::~SymbolPolygonGraphicsItem() noexcept
{
}
...
void SymbolPolygonGraphicsItem::setPosition(const Point& pos) noexcept
{
    QGraphicsItemGroup::setPos(pos.toPxQPointF());
}

void SymbolPolygonGraphicsItem::setRotation(const Angle& rot) noexcept
{
    QGraphicsItemGroup::setRotation(-rot.toDeg());
}

void SymbolPolygonGraphicsItem::setLength(const Length& length) noexcept
{
    mLineGraphicsItem->setLine(Point(0, 0), Point(length, 0));
    mTextGraphicsItem->setPosition(Point(length + Length(500000), Length(0)));
}

void SymbolPolygonGraphicsItem::setName(const QString& name) noexcept
{
    setToolTip(name);
...
QVariant SymbolPolygonGraphicsItem::itemChange(GraphicsItemChange change, const QVariant &value) noexcept
{
...
QPainterPath SymbolPolygonGraphicsItem::shape() const noexcept
{
