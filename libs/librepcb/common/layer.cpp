...
{
    mIsVisible = visible;
    foreach (IF_LayerObserver* object, mObservers) {
        object->layerHighlightColorChanged(*this, mIsVisible);
    }
...
void Layer::registerObserver(IF_LayerObserver& object) const noexcept
{
    mObservers.insert(&object);
}

void Layer::unregisterObserver(IF_LayerObserver& object) const noexcept
{
