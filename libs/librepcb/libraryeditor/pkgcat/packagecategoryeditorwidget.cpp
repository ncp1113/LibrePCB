...
PackageCategoryEditorWidget::PackageCategoryEditorWidget(workspace::Workspace& ws,
        LibraryEditor& editor, const FilePath& fp, QWidget* parent) throw (Exception) :
    EditorWidgetBase(ws, editor, fp, parent), mUi(new Ui::PackageCategoryEditorWidget)
{
    mUi->setupUi(this);
...
{
    PackageCategoryChooserDialog dialog(mWorkspace);
    if (dialog.exec()) {
...
void PackageCategoryEditorWidget::edtParentTextChanged(const QString& text) noexcept
{
    ...
    PackageCategoryTreeLabelTextBuilder textBuilder(db, mLocaleOrder, *mUi->lblParentCategories);
