    ...
    try {
        ...
        if (!category.isNull()) {
            ...
            if (!mEndlessRecursionUuid.isNull() && uuids.contains(mEndlessRecursionUuid)) {
                throw RuntimeError(__FILE__, __LINE__, QString(), tr("Endless recursion detected!"));
            }
        }
        return updateText(uuids, lastLine);
    } catch (const Exception& e) {
        setErrorText(e.getUserMsg());
        return false;
    }
}
...
void CategoryTreeLabelTextBuilder<ElementType>::setText(const QStringList& lines) noexcept
{
    ...
    for (int i = 0; i < lines.count(); ++i) {
        ...
        if (i == 0) {
            ...
            text.append(QString("<br>%1⤷ <b>%2</b>").arg(spaces, line));
        } else {
            text.append(QString("<br>%1⤷ %2").arg(spaces, line));
        }
    }
    setText(text);
}
...
template <>
FilePath CategoryTreeLabelTextBuilder<ComponentCategory>::getLatestCategory(const Uuid& category) const throw (Exception)
{
    return mDb.getLatestComponentCategory(category);
}

template <>
FilePath CategoryTreeLabelTextBuilder<PackageCategory>::getLatestCategory(const Uuid& category) const throw (Exception)
{
    return mDb.getLatestPackageCategory(category);
}

template <>
QList<Uuid> CategoryTreeLabelTextBuilder<ComponentCategory>::getCategoryParents(const Uuid& category) const throw (Exception)
{
    return mDb.getComponentCategoryParents(category);
}

template <>
QList<Uuid> CategoryTreeLabelTextBuilder<PackageCategory>::getCategoryParents(const Uuid& category) const throw (Exception)
{
    return mDb.getComponentCategoryParents(category);
}
